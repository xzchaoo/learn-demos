  ...
  private Single<SearchContext> doSearch(SearchContext rootSearchContext) {
    return Completable.defer(() -> {
      ...
      return c
        ...
        .doOnComplete(() -> {
          mergeContext(rootSearchContext, subContexts);
          executeMergedStrategy(rootSearchContext);
        });
      //.map(contexts2 -> mergeContext(rootSearchContext, subContexts))
      //.doOnSuccess(this::executeMergedStrategy);
    }).toSingleDefault(rootSearchContext);
  }
