...
public class Archaius2Test {
	...
	@Test
	public void test_replaceConfig() throws ConfigException {
		DefaultCompositeConfig dcc = new DefaultCompositeConfig();
		MapConfig mc1 = MapConfig.builder().put("a", 1).put("b", 2).build();
		MapConfig mc2 = MapConfig.builder().put("a", 11).put("b", 22).build();
		MapConfig mc3 = MapConfig.builder().put("a", 111).put("b", 222).build();
		...
		DefaultPropertyFactory.from(dcc).getProperty("a").asString("0").addListener(new DefaultPropertyListener<String>(){
			@Override
			public void onChange(String value) {
				System.out.println("onChange");
			}
		});
		dcc.addConfig("mc", mc1);
		dcc.replaceConfig("mc", mc2);
		dcc.replaceConfig("mc", mc2);
	}
