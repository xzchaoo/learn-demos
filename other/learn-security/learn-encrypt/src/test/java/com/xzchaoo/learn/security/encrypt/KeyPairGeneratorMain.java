...
public class KeyPairGeneratorMain {
	...
	public void test_DHdemo() throws Exception {
		...
		KeyPairGenerator kpgA = KeyPairGenerator.getInstance(ALG);
		//kpg1.initialize(1024);//DH算法要求size是 512~1024 的 64的倍数 默认是1024
		KeyPair kpA = kpgA.generateKeyPair();
		DHPublicKey publicKeyA = (DHPublicKey) kpA.getPublic();
		DHPrivateKey privateKeyA = (DHPrivateKey) kpA.getPrivate();

		KeyPairGenerator kpgB = KeyPairGenerator.getInstance(ALG);
		KeyPair kpB = kpgB.generateKeyPair();
		DHPublicKey publicKeyB = (DHPublicKey) kpB.getPublic();
		DHPrivateKey privateKeyB = (DHPrivateKey) kpB.getPrivate();

		//kpg2.initialize(publicKey21.getParams());//注意这里! 这个似乎可以不用

		//3. 甲生成密钥
		KeyAgreement ka1 = KeyAgreement.getInstance(ALG);
		ka1.init(privateKeyA);
		//通过各种方式获得B的公钥
		DHPublicKey publicKey12 = (DHPublicKey) KeyFactory.getInstance(ALG).generatePublic(new X509EncodedKeySpec(publicKeyB.getEncoded()));
		ka1.doPhase(publicKey12, true);
		SecretKey sk1 = ka1.generateSecret(ALG2);

		//4. 乙生成密钥
		KeyAgreement ka2 = KeyAgreement.getInstance(ALG);
		ka2.init(privateKeyB);
		//通过各种方式或的A的公钥
		DHPublicKey publicKey21 = (DHPublicKey) KeyFactory.getInstance(ALG).generatePublic(new X509EncodedKeySpec(publicKeyA.getEncoded()));
